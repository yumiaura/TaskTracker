...
- **`claude`** — карточки — это задачи самого Claude. Плагин говорит Claude — в начале каждой
  ...
  сам запрос ложится в **DONE** с пометкой `PROMPT`. Если же он вёл работу через MCP-задачу
  трекера, запрос приклеивается к ней: одна карточка с пометками `MCP` + `PROMPT`, а текст
  запроса — в разделе **MERGED CARDS**. Вопрос, на который он ответил чтением, карточки не
  создаёт.
...
Версия остаётся **0.0.3** до следующего релиза — почему и как изменение доходит до установленного плагина,
